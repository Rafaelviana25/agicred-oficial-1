...
const Dashboard: React.FC<DashboardProps> = ({ userProfile, onUpgradeSuccess }) => {
  ...
  return (
    <div className="flex h-screen overflow-hidden flex-col lg:flex-row uppercase font-bold text-slate-900">
      <aside className="w-56 glass-panel border-r border-slate-200 hidden lg:flex flex-col text-slate-500 z-50">
        <div className="p-6 flex flex-col gap-1">
          <AgicredLogo textClassName="text-xl text-slate-900" />
          <div className={`mt-2 px-3 py-1.5 rounded-lg flex flex-col items-center justify-center gap-1 ${isPro ? 'bg-violet-100 text-violet-700' : 'bg-emerald-100 text-emerald-700'}`}>
            <div className="flex items-center gap-2">
              {isPro ? <Crown size={14} className="fill-violet-700" /> : <User size={14} />}
              <span className="text-[10px] font-black tracking-widest uppercase">{isPro ? 'CONTA PRO' : 'CONTA GRÁTIS'}</span>
            </div>
            {isPro && userProfile?.pro_expires_at && (
              <div className="text-[8px] text-center w-full mt-1 border-t border-violet-200 pt-1">
        ...
        <nav className="flex-1 px-4 py-2 space-y-1">
          <SidebarItem icon={<LayoutDashboard size={18} />} label="INICIO" active={activeTab === 'overview'} onClick={() => setActiveTab('overview')} />
          <SidebarItem icon={<Users size={18} />} label="CLIENTES" active={activeTab === 'clients'} onClick={() => setActiveTab('clients')} />
          <SidebarItem icon={<Plus size={18} />} label="NOVA OPERAÇÃO" active={false} onClick={() => {
             if (isPro) {
          ...
          <SidebarItem icon={<AlertCircle size={18} />} label="VENCIDOS" active={activeTab === 'overdue'} onClick={() => setActiveTab('overdue')} />
          <SidebarItem icon={<FileText size={18} />} label="CONTRATOS" active={activeTab === 'contracts'} onClick={() => setActiveTab('contracts')} />
          <SidebarItem icon={<History size={18} />} label="LIQUIDADOS" active={activeTab === 'settled'} onClick={() => setActiveTab('settled')} />
        </nav>
        <div className="p-4 mt-auto">
          <button onClick={() => setShowProfile(true)} className="w-full flex items-center gap-2 p-2.5 bg-white rounded-xl border border-slate-200 hover:bg-slate-50 transition-all text-left shadow-sm">
            <div className="w-8 h-8 rounded-lg primary-gradient flex items-center justify-center text-white text-xs font-black uppercase shadow-lg shrink-0">
              {userProfile?.full_name?.[0]}
            </div>
            <div className="flex-1 overflow-hidden">
              <p className="text-slate-900 text-[10px] truncate uppercase font-black leading-tight">{userProfile?.full_name?.split(' ')[0]}</p>
              <div className="flex items-center gap-1 mt-0.5">
                 {isPro && <Crown size={8} className="text-violet-600 fill-violet-600" />}
                 <span className={`text-[7px] font-black tracking-widest uppercase ${isPro ? 'text-violet-600' : 'text-emerald-600'}`}>{isPro ? 'PRO ATIVO' : 'FREE'}</span>
              </div>
            </div>
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden h-full bg-slate-50">
        <header className="px-6 lg:px-8 h-16 lg:h-20 flex items-center justify-between shrink-0 glass-panel border-b border-slate-200 z-40 relative">
          <div className="flex items-center gap-4">
            <h1 className="text-xl font-black text-slate-900 tracking-tighter leading-none">AGICRED</h1>
            {!isPro && (
              <button 
                onClick={() => setShowUpgrade(true)}
                className="bg-violet-600 hover:bg-violet-700 text-white px-3 py-1.5 rounded-full flex items-center gap-1.5 transition-colors shadow-sm active:scale-95"
              >
                <Crown size={14} className="text-white" strokeWidth={2.5} />
                <span className="text-[9px] font-black tracking-widest uppercase">ATIVAR PRO</span>
              </button>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto pb-32 pt-0 no-scrollbar">
           {activeTab === 'overview' ? (
             ...
             <div className="max-w-4xl mx-auto px-3 lg:px-6 pt-3 space-y-3">
                {activeTab === 'clients' && (
                  <>
                    {!isPro && clients.length >= clientLimit && (
                      <div className="bg-gradient-to-r from-violet-600 to-indigo-600 rounded-2xl p-6 shadow-lg text-white relative overflow-hidden mb-4">
                        <div className="absolute top-0 right-0 opacity-10 transform translate-x-4 -translate-y-4">
                          <Crown size={120} />
                        </div>
                        <div className="relative z-10">
                          <h3 className="text-lg font-black uppercase tracking-tight mb-2 flex items-center gap-2"><Crown size={20} className="fill-white"/> LIMITE ATINGIDO</h3>
                          <p className="text-xs font-medium opacity-90 mb-4 leading-relaxed max-w-[85%]">
                            VOCÊ ATINGIU O LIMITE DE 3 CLIENTES DO PLANO GRÁTIS. FAÇA O UPGRADE PARA ADICIONAR MAIS CLIENTES E TER ACESSO ILIMITADO.
                          </p>
                          <button onClick={() => setShowUpgrade(true)} className="bg-white text-violet-600 px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-md hover:bg-violet-50 transition-colors flex items-center gap-2">
                ...
                {activeTab === 'contracts' && (
                  <>
                    ...
                    <button
                      ...
                      className="fixed bottom-20 right-6 w-12 h-12 bg-violet-600 rounded-full flex items-center justify-center text-white shadow-lg hover:bg-violet-700 transition-all active:scale-95 z-50"
                    >
                      <Plus size={20} />
                    </button>
                  </>
...
const OverviewView = ({ contracts, clients, setActiveTab, onAddClient, onAddContract, onSelectContract }: any) => {
  ...
  return (
    <div className="animate-in fade-in duration-500 uppercase font-black text-slate-900 pb-12">
      <div className="px-3 pt-6 max-w-5xl mx-auto">
        <div className="grid grid-cols-2 gap-2">
          <div className="glass-panel pt-2 pb-4 px-3 rounded-2xl border-2 border-rose-500 transition-all hover:bg-slate-50 flex flex-col justify-center text-center shadow-sm">
            <p className="text-slate-500 text-[10px] font-black tracking-widest uppercase mb-1 truncate">CAPITAL EMPRESTADO</p>
            <h4 className="text-[14px] md:text-[16px] font-black text-rose-500 tracking-tight leading-none truncate">
              R$ {totalLoaned.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </h4>
          </div>
          <div className="glass-panel pt-2 pb-4 px-3 rounded-2xl border-2 border-blue-500 transition-all hover:bg-slate-50 flex flex-col justify-center text-center shadow-sm">
            <p className="text-slate-500 text-[10px] font-black tracking-widest uppercase mb-1 truncate">CAPITAL A RECEBER</p>
            <h4 className="text-[14px] md:text-[16px] font-black text-blue-500 tracking-tight leading-none truncate">
              R$ {totalToReceive.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </h4>
          </div>
          <div className="glass-panel pt-2 pb-4 px-3 rounded-2xl border-2 border-emerald-500 transition-all hover:bg-slate-50 flex flex-col justify-center text-center shadow-sm">
            <p className="text-slate-500 text-[10px] font-black tracking-widest uppercase mb-1 truncate">JUROS JÁ RECEBIDOS</p>
            <h4 className="text-[14px] md:text-[16px] font-black text-emerald-500 tracking-tight leading-none truncate">
              R$ {totalInterestReceived.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </h4>
          </div>
          <div className="glass-panel pt-2 pb-4 px-3 rounded-2xl border-2 border-violet-500 transition-all hover:bg-slate-50 flex flex-col justify-center text-center shadow-sm">
            <p className="text-slate-500 text-[10px] font-black tracking-widest uppercase mb-1 truncate">JUROS A RECEBER</p>
            <h4 className="text-[14px] md:text-[16px] font-black text-violet-500 tracking-tight leading-none truncate">
      ...
      <div className="py-7 px-6 grid grid-cols-2 gap-4 max-w-lg mx-auto">
        <div className="text-center space-y-1.5">
          <p className="text-slate-500 text-[10px] font-black tracking-[0.2em] uppercase">PAGOS ESTE MÊS</p>
          <h2 className="text-2xl font-black text-emerald-500 tracking-tighter flex items-center justify-center">
            <span className="text-sm mr-1 font-bold text-emerald-500">R$</span>
            {totalPaidInMonth.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
          </h2>
        </div>
        <div className="text-center space-y-1.5">
          <p className="text-slate-500 text-[10px] font-black tracking-[0.2em] uppercase">PENDENTE ESTE MÊS</p>
          <h2 className="text-2xl font-black text-rose-500 tracking-tighter flex items-center justify-center">
            <span className="text-sm mr-1 font-bold text-rose-500">R$</span>
            {totalPendingInMonth.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
          </h2>
        </div>
      </div>

      <div className="px-5 space-y-4 max-w-lg mx-auto pb-24">
        <div className="grid grid-cols-2 gap-2.5">
          <button onClick={onAddContract} className="primary-gradient text-white py-3 rounded-full flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all hover:shadow-violet-500/25">
            <Plus size={15} /> <span className="text-[9px] font-black uppercase tracking-widest">NOVO CONTRATO</span>
          </button>
          <button onClick={onAddClient} className="bg-emerald-500 text-white py-3 rounded-full flex items-center justify-center gap-2 shadow-sm active:scale-95 transition-all hover:bg-emerald-600">
            <Users size={15} className="text-white" /> <span className="text-[9px] font-black uppercase tracking-widest">NOVO CLIENTE</span>
          </button>
        </div>

        <div className="glass-panel rounded-2xl overflow-hidden shadow-sm">
          <div className="p-3.5 border-b border-slate-200 flex justify-between items-center">
            <h3 className="text-rose-500 text-[11px] font-black tracking-tighter uppercase">
              CONTRATOS PENDENTES - <span className="text-slate-900">{monthsNames[currentMonthIdx].toUpperCase()}</span>
            </h3>
            <span className="text-[9px] bg-rose-500/10 text-rose-500 px-2 py-0.5 rounded-full font-black border border-rose-500/20">{activeInMonth.length}</span>
          </div>
          <div className="divide-y divide-slate-200">
            {activeInMonth.length > 0 ? activeInMonth.slice(0, 5).map((c: any) => (
              ...
              <div className="py-8 text-center"><p className="text-slate-500 text-[9px] font-black uppercase tracking-widest">SEM VENDAS PARA ESTE MÊS</p></div>
            )}
          </div>
          <button onClick={() => setActiveTab('contracts')} className="w-full bg-slate-50 py-3 text-slate-600 font-black text-[9px] uppercase tracking-[0.2em] border-t border-slate-200 hover:bg-slate-100 transition-colors">VER TODAS AS VENDAS</button>
        </div>

        <div className="glass-panel rounded-2xl overflow-hidden shadow-sm">
          <div className="p-3.5 border-b border-slate-200 flex justify-between items-center">
            <h3 className="text-emerald-500 text-[11px] font-black tracking-tighter uppercase">
              CONTRATOS PAGOS - <span className="text-slate-900">{monthsNames[currentMonthIdx].toUpperCase()}</span>
            </h3>
            <span className="text-[9px] bg-emerald-500/10 text-emerald-500 px-2 py-0.5 rounded-full font-black border border-emerald-500/20">{paidInMonth.length}</span>
          </div>
          <div className="divide-y divide-slate-200">
            {paidInMonth.length > 0 ? paidInMonth.slice(0, 5).map((c: any) => (
              ...
              <div className="py-8 text-center"><p className="text-slate-500 text-[9px] font-black uppercase tracking-widest">NENHUM RECEBIMENTO ESTE MÊS</p></div>
            )}
          </div>
          <button onClick={() => setActiveTab('settled')} className="w-full bg-slate-50 py-3 text-slate-600 font-black text-[9px] uppercase tracking-[0.2em] border-t border-slate-200 hover:bg-slate-100 transition-colors">VER HISTÓRICO DE PAGOS</button>
        </div>
      </div>
    </div>
  );
...
const SettledSection = ({ settledContracts, clients, onSelectContract }: any) => (
  <div className="space-y-2 animate-in fade-in duration-500 uppercase font-black text-slate-900 pb-20 max-w-lg mx-auto">
    <div className="flex justify-between items-center px-1 mb-1">
      <h3 className="text-[11px] font-black text-emerald-600 uppercase tracking-tighter">LIQUIDADOS</h3>
      <span className="text-[9px] text-emerald-600 font-black tracking-widest">{settledContracts.length} TÍTULOS</span>
    </div>
    <div className="flex flex-col gap-1.5">
      {settledContracts.map((c: any) => {
        const client = clients.find((cl: any) => cl.id === c.client_id);
        return (
          <div key={c.id} onClick={() => onSelectContract(c)} className="glass-panel px-3 py-3 rounded-2xl shadow-sm border-emerald-200 flex items-center justify-between group hover:bg-slate-50 transition-all border-l-4 border-l-emerald-500 cursor-pointer">
             <div className="flex-1 min-w-0 pr-2">
                <div className="flex items-center gap-1.5">
                   <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-tighter truncate group-hover:text-emerald-600 transition-colors leading-none">{client?.full_name}</h4>
                   <div className="w-4 h-4 bg-emerald-100 rounded-full flex items-center justify-center text-emerald-600 shrink-0"><CheckCircle2 size={11}/></div>
                </div>
                <div className="mt-1.5 flex items-center gap-2">
                   <p className="text-[8px] text-slate-500 uppercase font-black tracking-widest">PAGO EM {new Date(c.end_date + 'T12:00:00').toLocaleDateString()}</p>
                   <span className="text-slate-400">|</span>
                   <p className="text-[8px] text-slate-500 font-black uppercase">TÍTULO LIQUIDADO</p>
                </div>
             </div>
             
             <div className="flex flex-col items-end shrink-0">
                <p className="text-[12px] font-black text-slate-900 tracking-tighter leading-none">R$ {c.total_amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                <span className="text-[8px] text-emerald-600 font-black mt-1.5 uppercase group-hover:underline">VER RECIBO</span>
             </div>
          </div>
        );
      })}
...
const OverdueSection = ({ contracts, clients, onSelectContract, getOverdueStatus }: any) => (
  <div className="space-y-2 animate-in fade-in duration-500 uppercase font-black text-slate-900 pb-20 max-w-lg mx-auto">
     <div className="flex justify-between items-center px-1 mb-1">
        <h4 className="text-rose-600 font-black text-[11px] uppercase tracking-tighter leading-none px-1">INADIMPLENTES</h4>
        <span className="text-[9px] text-rose-600 font-black tracking-widest">{contracts.length} TÍTULOS</span>
     </div>
       ...
       <div className="flex flex-col gap-1.5">
          {contracts.map((c: any) => {
            ...
            return (
              <div 
                key={c.id} 
                className="glass-panel px-3 py-3 rounded-2xl shadow-sm border-rose-200 flex items-center justify-between group hover:bg-slate-50 transition-all border-l-4 border-l-rose-500 cursor-pointer overflow-hidden relative"
              >
                 <div onClick={() => onSelectContract(c)} className="flex-1 min-w-0 pr-2">
                    <div className="flex items-center gap-1.5">
                       <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-tighter truncate group-hover:text-rose-600 transition-colors leading-none">{client?.full_name}</h4>
                       <span className="text-rose-600 font-black text-[8px] uppercase tracking-widest bg-rose-100 px-1.5 py-0.5 rounded-md border border-rose-200 shrink-0">
                          {status?.diffDays}D
                       </span>
                    </div>
                    <div className="mt-1.5 flex items-center gap-2">
                       <p className="text-[8px] text-slate-500 uppercase font-black tracking-widest">PARC. #{status?.installmentNumber}</p>
                       <span className="text-slate-400">|</span>
                       <p className="text-[8px] text-slate-500 font-black uppercase">DÍVIDA: R$ {(Number(c.total_amount) - Number(c.paid_amount)).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                    </div>
                 </div>
                 
                 <div className="flex items-center gap-2.5 shrink-0">
                    <div className="text-right leading-none">
                       <p className="text-[12px] font-black text-rose-600 tracking-tighter">R$ {c.monthly_interest.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
                    </div>
                    <button 
                      onClick={(e) => {
                        e.stopPropagation();
                      ...
                      className="w-9 h-9 bg-slate-100 text-emerald-600 rounded-xl flex items-center justify-center shadow-lg active:scale-90 transition-all hover:bg-slate-200"
                    >
                       <Phone size={14} />
                    </button>
                 </div>
